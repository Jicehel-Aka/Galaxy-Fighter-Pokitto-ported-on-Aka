...
bool AkaRuntime::update(const Keys& k) {
    ...
    // Trace temporaire de diagnostic : etat brut a chaque CHANGEMENT (pas a
    // chaque frame, pour ne pas noyer les logs), pour voir precisement quels
    // bits materiels s'allument quand une touche physique est pressee (ex :
    // verifier si "C" active aussi, par erreur de cablage/mapping, RUN/MENU).
    static uint32_t s_lastRaw = 0xFFFFFFFF;
    if (s != s_lastRaw) {
        ESP_LOGI(TAG, "raw=0x%04lX  A=%d B=%d C=%d UP=%d DOWN=%d LEFT=%d RIGHT=%d RUN=%d MENU=%d",
                 (unsigned long)s,
                 (int)((s & GB_KEY_A) != 0), (int)((s & GB_KEY_B) != 0), (int)((s & GB_KEY_C) != 0),
                 (int)((s & EXPANDER_KEY_UP) != 0), (int)((s & EXPANDER_KEY_DOWN) != 0),
                 (int)((s & EXPANDER_KEY_LEFT) != 0), (int)((s & EXPANDER_KEY_RIGHT) != 0),
                 (int)((s & GB_KEY_RUN) != 0), (int)((s & GB_KEY_MENU) != 0));
        s_lastRaw = s;
    }
