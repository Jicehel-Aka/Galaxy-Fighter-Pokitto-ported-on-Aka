...
void CaptureController::move(bool &atTopOfScreen, bool &waitUntilComplete) {
    ...
    switch (this->capturedPlayer) {
        ...
        case CapturedPlayer::None:
            ...
            // AKA : garde ajoutee -- 'enemy' n'est jamais initialise avant le
            // premier appel a setEnemy() (aucune capture en cours). Le code
            // d'origine deferencait ce pointeur nul sans condition ; sur le
            // Pokitto reel (Cortex-M0) ceci lisait une adresse basse sans
            // planter (comportement indefini "silencieux") -- sur l'AKA
            // (ESP32-S3, protection memoire stricte), ca declenche un crash
            // (LoadProhibited) des l'entree en jeu.
            // If the enemy has just been killed then stop the sequence ..

            if (this->enemy && !this->enemy->isEnabled()) {
